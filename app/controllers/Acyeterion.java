...
/** 避孕药品发放登记. */
public class Acyeterion extends Controller {
	...
	public static void list(String keyword, Date date, String department, int page, int rows) {
		...
		for (models.Acyeterion mode : lists) {
			datas.add(mode.serialize());
		}
		result.put("page", page);
		result.put("rows", datas);
		result.put("total", models.Acyeterion.count());
		result.put("size", rows);
		renderJSON(result);
	}
	
	/** 获取当前操作者的全部数据. */
	private static List<models.Acyeterion> findAll(String keyword, Date date, int page, int rows) {
		List<models.Department> departments = Secure.getDepartments();
		List<models.Acyeterion> result = new ArrayList<models.Acyeterion>();
		int size = 0, offset = page * rows - rows, end = offset + rows;
		for (models.Department department : departments) {
			MorphiaQuery query = models.Acyeterion.find("byDepartment", department);
			if (keyword != null && !"".equals(keyword))
				query.filter("user", keyword);
			if (date != null)
				query.filter("date",  date);
			List<models.Acyeterion> data = query.asList();
			if (size >= offset && size < end) {
				for (models.Acyeterion flup : data) {
					result.add(flup);
					size ++;
				}
			} else {
	...
	/** 添加操作 */
	public static void add(String date, long size, String user, String department) {
	...
	public static void del(String id) {
		Map<String, Object> result = new HashMap<String, Object>();
		try {
			models.Acyeterion.findById(id).delete();
	...
	/** 修改操作 */
	public static void update(String id, String date, long size, String user, String department) {
		Map<String, Object> result = new HashMap<String, Object>();
		if (department == null || "".equals(department)) {
			result.put("error", "记录添加失败，请选择该所属部门。<br>如果还未创建部门，请先创建部门后进行添加");
			renderText(JSON.serialize(result));
